...
MODELSCOPE_DATASET_REPO = os.getenv(
    "MODELSCOPE_DATASET_REPO",
    "hf/angrygiraffe-claude-opus-4.6-4.7-reasoning-8.7k",
)
MODELSCOPE_DATASET_URL = f"https://modelscope.cn/datasets/{MODELSCOPE_DATASET_REPO}"
MODELSCOPE_DATASET_REVISION = os.getenv("MODELSCOPE_DATASET_REVISION", "master")
MODELSCOPE_DATASET_FILE = os.getenv("MODELSCOPE_DATASET_FILE", "full_train.jsonl")
DATASET_PATH = Path("datasets/claude-opus-4.6-4.7-reasoning-8.7k")
FORCE_DATASET_DOWNLOAD = os.getenv("FORCE_DATASET_DOWNLOAD", "0") == "1"
...
def download_if_needed(url: str, local_path: Path, force: bool = False) -> None:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "trio-case-sft-distillation"})
...
def load_training_dataset(
    ...
    local_file = dataset_path / dataset_file
    download_if_needed(
        modelscope_file_url(dataset_repo, dataset_revision, dataset_file),
        local_file,
        FORCE_DATASET_DOWNLOAD,
    )

    print(f"Loading ModelScope dataset file: {local_file}")
    return load_dataset(
        "json",
        data_files=str(local_file),
        split="train",
        cache_dir=str(dataset_path / ".datasets_cache"),
    )
...
def load_examples(
    ...
    dataset = load_training_dataset(dataset_repo, dataset_revision, dataset_file, dataset_path)
...
def main() -> None:
    ...
    examples = load_examples(
        dataset_path,
        MODELSCOPE_DATASET_REPO,
        MODELSCOPE_DATASET_REVISION,
        MODELSCOPE_DATASET_FILE,
    )
    ...
    swanlab_init_kwargs = {
        ...
        "config": {
            "base_model": BASE_MODEL,
            "dataset_url": MODELSCOPE_DATASET_URL,
            "dataset_repo": MODELSCOPE_DATASET_REPO,
            "dataset_revision": MODELSCOPE_DATASET_REVISION,
            "dataset_file": MODELSCOPE_DATASET_FILE,
